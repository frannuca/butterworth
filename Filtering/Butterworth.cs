...
internal class IIR_ButterworthFilter 
{
    //Global variables
    double fs = 2;
    double u_f1;
    ...
    double[][]
        save_filt_coeff; //Matrix where to save the numerator and denominator. First row is the numerator; second row is the denominator
    ...
    public Complex[] Char_poly(Matrix<Complex> temp_matr_poly, int row_col)
    {
...

        {
            ...
            else
            {
                ...
                try
                {
                    num_det = factorial(row_col) /
                              (factorial(row_col - kk) * factorial(kk)); //Calculate the number of combinations   
                }
    ...
    //Step 5: Use Bilinear transformation to find discrete equivalent
    public void Bilinear(Matrix<Complex> a_arma_f, Matrix<Complex> b_arma_f, Matrix<Complex> c_arma_f,
        Matrix<Complex> d_arma_f, double fs_f, int type_filt_f)
    {
        double t_arma;
        double r_arma;
    ...
    public double[] Filter_Data(double[][] coeff_filt, double[] pre_filt_signal)
    {
        ...
        for (int kk = 0; kk < pre_filt_signal.Length; kk++)
        {
            ...
            else
            {
                ...
                for (int ww = 1; ww < coeff_filt[0].Length; ww++)
                {
                    w_val[ww - 1][kk] = pre_filt_signal[kk] * coeff_filt[0][ww] + w_val[ww][kk - 1] -
                                        filt_signal[kk] * coeff_filt[1][ww];

                    if (ww == coeff_filt[0].Length - 1)
                    {
                        w_val[ww - 1][kk] = pre_filt_signal[kk] * coeff_filt[0][ww] -
                                            filt_signal[kk] * coeff_filt[1][ww];
                    }
                }
            }
